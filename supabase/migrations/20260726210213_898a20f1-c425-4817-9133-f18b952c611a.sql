...
USING (
  ...
  OR (werkplaats_rol() = 'schadeherstel' AND discipline = 'spuit' AND status = ANY (ARRAY['aangevraagd','ingepland','bezig','afgerond']))
  OR (werkplaats_rol() = 'poetser' AND discipline = 'poets' AND status = ANY (ARRAY['aangevraagd','ingepland','bezig','afgerond','goedgekeurd']))
);
...
WITH CHECK (
  ...
  OR (werkplaats_rol() = 'schadeherstel' AND discipline = 'spuit' AND assigned_to = auth.uid() AND status = ANY (ARRAY['bezig','afgerond']))
  ...
  OR (werkplaats_rol() = 'poetser' AND discipline = 'poets' AND status = ANY (ARRAY['bezig','goedgekeurd']))
);