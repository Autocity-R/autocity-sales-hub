-- 1. Data-fix: markeer signature-rijen als signed voor contracten die al 'getekend' zijn
UPDATE public.contract_signatures cs
SET signed_at = COALESCE(cs.signed_at, cd.signed_at, now())
FROM public.contract_documents cd
WHERE cs.contract_id = cd.id
  AND cd.status = 'getekend'
  AND cs.signed_at IS NULL;

-- 2. Harden get_contract_by_token: ook al ondertekend als contract_documents.status = 'getekend'
...
BEGIN
  ...
  END IF;
  IF v_sig.signed_at IS NOT NULL OR v_doc.status = 'getekend' THEN
