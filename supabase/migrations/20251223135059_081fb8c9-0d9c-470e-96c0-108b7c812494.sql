-- Fix: Remove ::text casts from UUID comparisons in auto_register_damage_repair_on_completion
-- This fixes the "operator does not exist: uuid = text" error when completing schadeherstel tasks
...
BEGIN
  ...
  IF NEW.category = 'schadeherstel' 
    ...
    -- Check of record al bestaat (voorkom duplicaten) - FIX: gebruik UUID direct, geen ::text cast
    IF NOT EXISTS (
      SELECT 1 FROM damage_repair_records 
      WHERE task_id = NEW.id::text
    ) THEN
      -- Insert damage repair record - FIX: cast NEW.id naar text voor task_id kolom
      INSERT INTO damage_repair_records (
        ...
        NEW.id::text,  -- task_id is text in damage_repair_records table
        NEW.vehicle_id,
