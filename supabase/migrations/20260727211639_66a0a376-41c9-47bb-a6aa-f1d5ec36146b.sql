ALTER TABLE public.workshop_invoices
  ADD COLUMN IF NOT EXISTS invoice_kind text NOT NULL DEFAULT 'extern',
  ADD COLUMN IF NOT EXISTS vehicle_id uuid,
  ADD COLUMN IF NOT EXISTS source_work_order_ids jsonb NOT NULL DEFAULT '[]'::jsonb;

CREATE INDEX IF NOT EXISTS idx_workshop_invoices_kind ON public.workshop_invoices (invoice_kind, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uniq_internal_invoice_per_work_order
  ON public.workshop_invoices (work_order_id)
  WHERE invoice_kind = 'intern' AND work_order_id IS NOT NULL;
...
BEGIN
  ...
  ELSE
    ...
    FOR r IN
      SELECT w.id, w.part, w.description
...
DROP TRIGGER IF EXISTS trg_internal_invoice_on_approval ON public.work_orders;
CREATE TRIGGER trg_internal_invoice_on_approval
AFTER UPDATE ON public.work_orders
FOR EACH ROW EXECUTE FUNCTION public.create_internal_invoice_on_approval();