ALTER TABLE public.work_orders
  ADD COLUMN IF NOT EXISTS uitvoering text NOT NULL DEFAULT 'intern',
  ADD COLUMN IF NOT EXISTS extern_party text,
  ADD COLUMN IF NOT EXISTS extern_dropped_at timestamptz,
  ADD COLUMN IF NOT EXISTS extern_returned_at timestamptz,
  ADD COLUMN IF NOT EXISTS extern_cost numeric;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'work_orders_uitvoering_check') THEN
    ALTER TABLE public.work_orders
      ADD CONSTRAINT work_orders_uitvoering_check CHECK (uitvoering IN ('intern','extern'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_work_orders_uitvoering ON public.work_orders (discipline, uitvoering, status);

ALTER TABLE public.werkplaats_tarieven
  ADD COLUMN IF NOT EXISTS eigen_schadeherstel boolean NOT NULL DEFAULT true;

-- Interne doorbelasting alleen voor eigen uitvoering
...
BEGIN
  ...
  IF COALESCE(NEW.origin, 'intern') <> 'intern' OR NEW.external_customer IS NOT NULL THEN
    RETURN NEW;
  END IF;
  IF COALESCE(NEW.uitvoering, 'intern') <> 'intern' THEN
    RETURN NEW;
  END IF;
  ...
  ELSE
    IF EXISTS (
      ...
      WHERE vehicle_id = NEW.vehicle_id
        ...
        AND COALESCE(uitvoering, 'intern') = 'intern'
    ...
    FOR r IN
      ...
      WHERE w.vehicle_id = NEW.vehicle_id
        ...
        AND COALESCE(w.uitvoering, 'intern') = 'intern'
...
-- Rol 'schadeherstel' mag geen uitbestede opdrachten aanraken
CREATE OR REPLACE FUNCTION public.guard_extern_uitvoering()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $function$
BEGIN
  IF public.werkplaats_rol() = 'schadeherstel'
     AND (COALESCE(OLD.uitvoering, 'intern') = 'extern' OR COALESCE(NEW.uitvoering, 'intern') = 'extern') THEN
    RAISE EXCEPTION 'Uitbesteed schadeherstel wordt door de leidinggevende beheerd.';
  END IF;
  RETURN NEW;
END;
$function$;

DROP TRIGGER IF EXISTS trg_guard_extern_uitvoering ON public.work_orders;
CREATE TRIGGER trg_guard_extern_uitvoering
  BEFORE UPDATE ON public.work_orders
  FOR EACH ROW EXECUTE FUNCTION public.guard_extern_uitvoering();