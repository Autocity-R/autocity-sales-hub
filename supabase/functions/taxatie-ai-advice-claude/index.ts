...
const systemPrompt = `Je bent een ervaren auto-inkoper bij Autocity. Je denkt als een HANDELAAR, niet als een data-analist.
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('📊 Taxatie request received (Claude):', {
    ...
    console.log('🤖 Sending to Claude with learning context...');
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        tools: [adviceToolSchema],
        tool_choice: { type: 'tool', name: 'generate_taxatie_advice' },
      }),
    });
    ...
    const toolUse = (data.content || []).find((b: any) => b.type === 'tool_use' && b.name === 'generate_taxatie_advice');
    let advice: any;

    if (toolUse && toolUse.input) {
      advice = toolUse.input;
    } else {
      ...
      advice = parseClaudeResponse(textBlocks);
      if (!advice) {
        throw new Error('Invalid response from Claude');
      }
    }

    console.log('📋 Taxatie advice generated with learning (Claude):', {
    ...
    console.error('❌ Error in taxatie-ai-advice-claude:', error);
    return new Response(JSON.stringify({
