...
export class SupabaseInventoryService {
  ...
  async getAllVehicles(): Promise<Vehicle[]> {
    try {
      const { data, error } = await supabase
        .from('vehicles')
        .select('*')
        .not('status', 'in', '(afgeleverd,leenauto,extern_werkplaats)')
  ...
  async updateVehicle(vehicle: Vehicle): Promise<Vehicle> {
    ...
    const details = {
      ...
      bpmReportSent: vehicle.bpmReportSent ?? incomingDetails.bpmReportSent ?? existingDetails.bpmReportSent ?? false,
      bpmReportSentDate: vehicle.bpmReportSentDate ? vehicle.bpmReportSentDate.toISOString() : (incomingDetails.bpmReportSentDate ?? existingDetails.bpmReportSentDate ?? null),
      papersReceived: vehicle.papersReceived ?? incomingDetails.papersReceived ?? existingDetails.papersReceived ?? null,
